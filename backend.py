...
async def chat_with_claude(message: dict):
    ...
    try:
        ...
        query_kw = [
            ...
            "cuánto", "cuanto", "total", "resumen", "estado",
            "hay en", "tenemos en", "tienes", "tienes en",
        ]
        ...
        try:
            ...
            if df is not None and not df.empty:
                resumen_lines = ["FACTURAS REGISTRADAS EN EL SISTEMA:"]
                for mes_col in app_module.MESES:
                    df_mes = df[df.get("Mes", pd.Series()) == mes_col] if "Mes" in df.columns else pd.DataFrame()
                    if not df_mes.empty:
                        total_val = 0
                        try:
                            total_val = pd.to_numeric(
                                df_mes["Valor Total"].astype(str)
                            ...
                            pass
                        proveedores = df_mes["Proveedor"].dropna().unique().tolist()[:5] if "Proveedor" in df_mes.columns else []
                        prov_str = ", ".join(str(p) for p in proveedores)
                        resumen_lines.append(
                            f"- {mes_col}: {len(df_mes)} facturas | Total: ${total_val:,.0f} | "
                            f"Proveedores: {prov_str}{'...' if len(proveedores) == 5 else ''}"
                        )
