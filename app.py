...
COLUMNS = [
    # (el resto de COLUMNS va aquí — no visible en el diff, mantener original)
]
...
def process_emails_for_month(mes_nombre: str, year: int = None):
    ...
    for eid in email_ids:
        try:
            ...
            print(f"❌ Error procesando correo {eid}: {e}")
            continue

    mail.logout()
    print(f"✅ Procesamiento completado. Facturas encontradas: {facturas_encontradas}")
...
def _validate_invoice_data(datos: dict, subject: str = "") -> tuple[bool, str]:
    ...
    try:
        valor_total = float(datos.get("valor_total", 0) or 0)
    except (ValueError, TypeError):
        valor_total = 0.0

    # 1. El número no puede estar vacío
    if not numero or numero.upper() in ("N/A", "NA", "NONE", "NULL", ""):
        return False, f"Número de factura vacío o inválido: '{numero}'"

    # 2. Número no debe coincidir con patrones de links/pagos
